...
HAL_StatusTypeDef HAL_SD_WriteBlocks(SD_HandleTypeDef *hsd,
		const uint8_t *pData, uint32_t BlockAdd, uint32_t NumberOfBlocks,
		uint32_t Timeout)
{
	SDMMC_DataInitTypeDef config;
	uint32_t errorstate;
...
void HAL_SD_IRQHandler(SD_HandleTypeDef *hsd)
{
	...
	else if (__HAL_SD_GET_FLAG(hsd, SDMMC_FLAG_DATAEND) != RESET)
	{
		...
		__HAL_SD_DISABLE_IT(hsd,
				SDMMC_IT_DATAEND | SDMMC_IT_DCRCFAIL | SDMMC_IT_DTIMEOUT | SDMMC_IT_TXUNDERR | SDMMC_IT_RXOVERR | SDMMC_IT_TXFIFOHE | SDMMC_IT_RXFIFOHF);
		...
		HAL_SD_CardCSDTypeDef *pCSD)
{
	...
	if (hsd->SdCard.CardType == CARD_SDSC)
	{
		...
		pCSD->MaxRdCurrentVDDMin =
				(uint8_t) ((hsd->CSD[2] & 0x38000000U) >> 27U);

		pCSD->MaxRdCurrentVDDMax =
				(uint8_t) ((hsd->CSD[2] & 0x07000000U) >> 24U);

		pCSD->MaxWrCurrentVDDMin =
				(uint8_t) ((hsd->CSD[2] & 0x00E00000U) >> 21U);

		pCSD->MaxWrCurrentVDDMax =
				(uint8_t) ((hsd->CSD[2] & 0x001C0000U) >> 18U);
		...
		HAL_SD_CardStatusTypeDef *pStatus)
{
	...
	else
	{
		...
		pStatus->ProtectedAreaSize = (((sd_status[1] & 0xFFU) << 24U)
				| ((sd_status[1] & 0xFF00U) << 8U)
		...
		pStatus->AllocationUnitSize = (uint8_t) ((sd_status[2] & 0xF00000U)
				>> 20U);
		...
		pStatus->VideoSpeedClass = (uint8_t) ((sd_status[4] & 0xFF000000U)
				>> 24U);
	}
		...
		uint32_t SpeedMode)
{
  ...
  else
  {
	...
	case SDMMC_SPEED_MODE_AUTO:
	{
		...
		{
			/* Enable High Speed */
			if (SD_SwitchSpeed(hsd,
			SDMMC_SDR25_SWITCH_PATTERN) != HAL_SD_ERROR_NONE)
			{
				hsd->ErrorCode |= HAL_SD_ERROR_UNSUPPORTED_FEATURE;
				status = HAL_ERROR;
			}
...
HAL_StatusTypeDef HAL_SD_Abort(SD_HandleTypeDef *hsd)
{
	...
	if (hsd->State == HAL_SD_STATE_BUSY)
	{
		...
		{
			while (!__HAL_SD_GET_FLAG(hsd,
					SDMMC_FLAG_DABORT | SDMMC_FLAG_DATAEND))
			{
				if ((HAL_GetTick() - tickstart) >= SDMMC_SWDATATIMEOUT)
				{
					hsd->ErrorCode = HAL_SD_ERROR_TIMEOUT;
...
static uint32_t SD_InitCard(SD_HandleTypeDef *hsd)
{
	...
	errorstate = SDMMC_CmdSelDesel(hsd->Instance,
			(uint32_t) (((uint32_t) hsd->SdCard.RelCardAdd) << 16U));
	if (errorstate != HAL_SD_ERROR_NONE)
	{
		return errorstate;
	}
